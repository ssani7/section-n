	...
	return (
		<div className={`navbar ${navBg ? 'bg-base-300' : 'bg-transparent'} fixed top-0 z-40 px-3 md:px-6 transition-all duration-700`}>
			<div className="flex-1">
				<Link to="/" className="font-bold normal-case text-lg md:text-3xl great-vibes">
					Section N
				</Link>
			</div>
			<div className="flex justify-end w-2/3 md:w-1/2 xl:w-1/4">
				...
				<div className="dropdown dropdown-end flex items-center mx-3 md:mx-5">
					<label className="swap swap-rotate">
						...
						<svg className="swap-on fill-current w-5 h-5 md:h-8 md:w-8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
						</svg>

						<svg className="swap-off fill-current w-5 h-5 md:h-8 md:w-8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
					...
					{userData || user ? (
						<div className="dropdown dropdown-end">
							...
							<ul onClick={(e) => e.target.blur()} tabIndex="0" className="menu menu-compact dropdown-content mt-3 p-2 shadow bg-base-100 rounded-box w-52">
								...
								<li>{userData?.portfolio ? <Link to={`/userPortfolio/${userData?.email}/self`}>Portfolio</Link> : <Link to="/editPortfolio">Create Portfolio</Link>}</li>
								<li>
						...
						<div className="dropdown dropdown-end">
							<Link className="text-sm md:text-xl font-bold my-auto whitespace-nowrap" to="/login">
								Sign In
							</Link>
						</div>
					)}
