	...
	return (
		<div className="w-full h-72 md:min-h-screen mx-auto relative overflow-hidden">
			{slides.map((slide, index) => (
				<div
					key={index}
					className={`absolute w-full h-full right-0 transform ${index > currentSlide && 'translate-x-full'} ${index < currentSlide && '-translate-x-full'} transition-all duration-700`}>
			...
			<motion.h2
				...
				className="text-white text-3xl text-center md:text-7xl  opacity-100 capitalize great-vibes absolute bottom-5 lg:bottom-28 left-0 right-0 z-30 drop-shadow-lg shadow-black">
				Welcome To The Coolest Section in Town
			</motion.h2>
			<div className="absolute flex justify-center transform md:right-5 right-2 bottom-1 md:bottom-5 z-30">
				<button onClick={previousSlide} className="text-white h-8 w-8 px-2 border rounded-full hover:bg-white hover:text-black mr-1 md:mr-4">
					❮
				</button>
				<button onClick={nextSlide} className="text-white h-8 w-8 border rounded-full hover:bg-white hover:text-black">
					❯
				</button>
			</div>
