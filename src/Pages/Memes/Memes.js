...
import { default as haha, default as noReact } from '../../images/icons/laughing.png';
		...
		<div className="pt-20 bg-base-100">
			...
			<div className="mt-10 max-w-full mx-6 md:max-w-5xl md:mx-auto shadow-lg p-5 md:p-10 rounded-xl flex flex-col items-center bg-base-300">
					...
					<>
						{media?.fileType === 'image' ? (
							<div className="h-80 w-9/12 my-5">
								<img onLoad={() => setLoadingData({ ...loadingData, uploadingMedia: false })} className="w-full h-full object-contain" src={media.media} alt="" />
							</div>
						) : (
				...
				{loadingData?.uploadingMedia ? (
					...
					<span onClick={() => mediaRef.current.click()} className="btn btn-outline capitalize ">
				...
				{loadingData?.postingMeme ? (
					...
					<button disabled={media.fileType || caption ? false : true} onClick={() => post()} className="w-full mt-5 btn btn-primary capitalize">
						Post
					</button>
				)}
			</div>
			...
			{posts.map((post, index) => (
				<div ref={(el) => (scrollRef.current[index] = el)} className="w-full h-fit pt-6 px-4 md:max-w-5xl md:mx-auto mt-10 bg-base-200 md:rounded-xl">
					...
					<div className="w-full border md:rounded-lg border-base-content p-3 flex items-center justify-between md:justify-evenly px-10 md:px-5 relative">
						<div className="flex items-center group no-menu select-none">
							...
							<input onChange={(e) => setOpenReact({ ...openReact, [post._id]: e.target.checked })} checked={openReact[post._id]} className="hidden" />

							<div
								className={`absolute z-30 flex w-fit left-0 rounded-2xl items-center justify-center bg-white bottom-full p-2 md:px-5 mb-4 transition duration-75 ${
									openReact[post._id] ? 'visible translate-y-0' : 'invisible translate-y-5'
								}`}>
