...
const Courses = () => {
	...
	return (
		<div className="pt-20 min-h-screen">
				...
				<div className="drawer-side border-r border-primary h-full">
					...
					<ul className="menu p-4 overflow-y-auto w-80 bg-base-100 text-base-content">
						{semesters?.map((semester, i) => (
							<li key={i} onClick={() => setChecked(false)}>
			...
			<label htmlFor="course-drawer" className={`btn btn-primary btn-circle drawer-button lg:hidden fixed bottom-10 left-4 z-40 transform transition-all ${checked ? 'rotate-180' : 'rotate-0'}`}>
				❯
			</label>
			<Footer />
		</div>
