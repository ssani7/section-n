...
const Stars = () => {
	...
	return (
		<div className={`text-center ${length === 'all' && 'pt-20'} bg-base-100`}>
			...
			<div ref={ref} className="pb-10 grid grid-cols-2 gap-2 md:gap-5 mx-4 lg:mx-auto w-fit">
				{stars.map((star, i) => (
					<motion.div
						...
						className="hero py-2 bg-base-200 max-w-3xl rounded-xl shadow-lg"
						variants={{
						...
						<div className={`hero-content flex-col ${(i + 1) % 2 === 0 ? 'lg:flex-row-reverse' : 'lg:flex-row'} w-full justify-evenly h-full`}>
							...
							<div className="h-1/2 md:h-auto lg:w-1/3 lg:mt-0">
								<motion.h1 variants={item} className="text-lg md:text-3xl font-bold poppins">
									{star?.name}
								</motion.h1>
								<motion.p variants={item} className="text-xs md:text-lg mt-4 font-semibold">
									{star.achievement}
								</motion.p>
							</div>
						</div>
			...
			{length === 'all' ? (
				<button onClick={() => navigate('/addAchievement')} className="btn btn-primary mb-6 normal-case">
					Add Your Achievements
				</button>
			) : (
				<button onClick={() => navigate('/achievements/all')} className="btn btn-primary mb-6 normal-case">
