...
const VerifyAcc = () => {
		...
		return (
			<>
				<h1 className="text-2xl font-bold text-center">
					Hey {userData?.displayName}! <br /> You are a verified <s>member</s> Celebrity of Section N
				</h1>
	...
	const onSubmit = async (data) => {
		...
		await axios.put(`https://section-n-server.vercel.app/user/verify?id=${data?.id}&email=${userData?.email}`).then((res) => {
	...
	return (
		<div className="w-full text-center">
			...
			<form className="pt-12 max-w-xs md:max-w-2xl mx-auto" onSubmit={handleSubmit(onSubmit)}>
				...
				{fetching ? (
					<button className="btn loading w-full md:w-auto" value="submit">
						Updating
					</button>
				) : (
