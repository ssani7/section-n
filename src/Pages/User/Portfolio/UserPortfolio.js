...
const UserPortfolio = () => {
	...
	const onSubmit = (data) => {
		...
		setUploading(true);
		emailjs
			.send('google', 'template_xap02hr', template, 'Gk24mox0snUD7ToWx')
			.then((res) => {
				console.log(res);
				setUploading(false);
			...
			<motion.div
				...
				<div className="absolute flex flex-col justify-center items-center transform left-0 right-0 bottom-0 bg-black w-full h-full bg-opacity-50 whitespace-nowrap z-30">
					<motion.h2
						...
						className="text-white text-2xl text-center md:text-5xl 2xl:text-8xl opacity-100 capitalize great-vibes drop-shadow-lg shadow-black">
						{portfolio?.heading1}
					</motion.h2>
						...
						<div className="flex justify-center w-full h-full" ref={ref}>
							<motion.div
								...
								<div className="lg:w-2/3 md:pl-12 md:py-10 p-8 text-base-content">
									...
									<a href={portfolio?.cv || undefined} target="_blank" className="cursor-pointer link-hover" rel="noreferrer">
								...
								<motion.img
									...
									className="z-20 h-60 object-top object-cover w-2/3 md:w-1/3 xl:w-1/4 right-0 shadow-2xl rounded-full mt-3 md:m-0  md:-right-10 md:rounded-lg md:h-auto md:max-h-full md:absolute "
									alt=""
								/>
							</motion.div>
						</div>
				...
				{({ inView, ref, entry }) => (
					<div className="grid grid-cols-1 lg:grid-cols-3 justify-center gap-10 mx-auto w-full my-20 py-10 border-y" ref={ref}>
						{portfolio?.statistics?.map((stat, i) => (
							<CountUp key={i} end={inView ? stat.number : 0} duration={1.8}>
								{({ countUpRef }) => (
						...
						{({ inView, ref }) => (
							<motion.div ref={ref} initial="hidden" animate={`${inView && 'visible'}`} variants={container} className="card max-w-sm bg-base-200 shadow-xl py-10 px-5 mx-auto h-full w-full">
								...
								<div className="card-body items-center text-center">
									<motion.h2 variants={item} className="card-title">
										{service.serviceName}
									</motion.h2>
							...
							{portfolio?.skills?.map((skill, i) => (
								<div key={i} className="border max-w-xl mx-auto p-6">
									<div className="flex items-center justify-between">
										<h1 className="text-xl md:text-2xl text-left pr-5 poppins">{skill.skillName}</h1>
				...
				<div className="my-20 overflow-y-hidden">
					...
					<div className="my-20 grid grid-cols-1 lg:grid-cols-3 gap-10 mx-6 lg:mx-20">
						{portfolio?.projects?.map((project, i) => (
							<InView key={i} threshold={0.3} triggerOnce={false}>
								{({ inView, ref, entry }) => (
									<div ref={ref}>
										<motion.div
											variants={container}
											...
											key={i}
											className="hero rounded-xl shadow-2xl h-full relative group cursor-pointer overflow-y-hidden">
											<motion.img variants={item} src={project.image} className="rounded-xl lg:mx-12 absolute w-full z-10 h-full object-cover" alt="" />

											<div
												className={` hero-content px-8 py-10 rounded-xl flex-col z-20 transform duration-300 transition-all invisible translate-y-80 group-hover:visible group-hover:bg-black text-white group-hover:bg-opacity-50 group-hover:translate-y-0 w-full h-full`}>
									...
									<motion.div
										...
										<motion.div class="card-body items-center text-center p-5">
											<motion.h2 variants={item} className="mt-2">
												{[...Array(parseInt(review.rating)).keys()].map((star, i) => (
													<FontAwesomeIcon icon={faStar} className="text-yellow-300 mx-0.5" />
												))}
												{[...Array(5 - parseInt(review.rating)).keys()].map((star, i) => (
													<FontAwesomeIcon icon={faStar} className="text-neutral mx-0.5" />
												))}
											</motion.h2>

											<motion.h2 variants={item} class="card-title poppins">
												{review.clientName || 'No Client Name Found'}
											</motion.h2>
											<motion.h2 variants={item} class="text-sm">
												{review.clientCompany || 'No Company Found'}
											</motion.h2>
											<motion.p variants={item} className="mt-3">
												{review.review || 'No Client Review Found'}
											</motion.p>
										</motion.div>
									</motion.div>
								)}
						...
						<motion.div ref={ref} initial="hidden" animate={`${inView && 'visible'}`} variants={container} className="w-full flex justify-center items-center my-20">
							<div className="card w-full md:w-3/5 lg:w-3/5 bg-base-100 shadow-xl lg:px-10 mx-3 pb-6">
								<div className="card-body w-full py-4">
									<motion.h2 variants={item} className="card-title text-3xl font-bold justify-center mb-6">
										Send Me an Email
									</motion.h2>
									<form onSubmit={handleSubmit(onSubmit)}>
										<motion.div variants={item} className="form-control w-full lg:max-w-2xl">
											<input
												...
												className="input input-bordered mt-6 text-xl w-full h-14"
												{...register('from_email', { required: 'Email address is required to send an email' })}
											/>
										...
										{uploading ? (
											<span variants={item} className="btn btn-primary w-full loading mt-4 normal-case">
												Sending
											</span>
										) : (
									...
									<div className="flex flex-col md:flex-row items-center justify-between mt-5">
										...
										<div className="my-5 flex justify-evenly w-1/4">
											<a target="_blank" href={portfolio?.fb} rel="noreferrer">
												<FontAwesomeIcon className="h-8 w-8 m-3 hover:text-blue-600 hover:scale-125 transition-all rounded-full border-2 p-2 hover:border-blue-600" icon={faFacebookF} />
											</a>
											<a target="_blank" href={portfolio?.linkedin} rel="noreferrer">
												<FontAwesomeIcon className="h-8 w-8 m-3 hover:text-blue-600 hover:scale-125 transition-all rounded-full border-2 p-2 hover:border-blue-600" icon={faLinkedin} />
											</a>
											<a target="_blank" href={portfolio?.twitter} rel="noreferrer">
												<FontAwesomeIcon className="h-8 w-8 m-3 hover:text-blue-600 hover:scale-125 transition-all rounded-full border-2 p-2 hover:border-blue-600" icon={faTwitter} />
											</a>
										</div>
									</div>
								</div>
